...
def type2_upsert(microBatchDF, batch):
    ...
    sql_query = """
        ...
        USING (
            -- les books_id présents dans le microBatch (= Bronze): si le book_id est absent dans books_silver, (="NOT MATCHED"), on insere la ligne. Sinon, on met à jour la ligne (="MATCHED") en l'historisant

            SELECT updates.book_id as merge_key, updates.*
            FROM updates

            UNION ALL
            -- on remet à nouveau les lignes du microBatch (si le prix a évolué) : cette ligne sera prise pour l'insert (="NOT MATCHED")
        ...
        -- on met à jour l'ancienne ligne dans books_silver
        WHEN MATCHED AND books_silver.current = true AND books_silver.price <> staged_updates.price THEN
          UPDATE SET current = false, end_date = staged_updates.updated
        
        --  insert de ce book_id dans books_silver (dernière version du book_id)
...
# on est comme pour orders_silver toujours en output = Append 
def process_books():
